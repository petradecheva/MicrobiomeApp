    ...
    data <- reactive({
          ...
          taxonomy <- read_tsv(input$probe3$datapath) %>%
            ...
            separate(taxonomy,
                     into=c("kingdom", "phylum", "class", "order", "family", "genus"),
                     sep=";")
>>>>>>> 6cd0690667934a7086f37b2e2dff6a1633de8c0a
      ...
      barchart <- function(){
        ...
        geom_col() +
          ...
          theme(axis.text.x = element_markdown(),
                legend.text = element_markdown(),
                legend.key.size = unit(10, "pt")) + 
          scale_fill_manual(values = v2, labels=paste(data2()$taxon, str_replace_all(paste(round(data2()$mean_rel_abund, 1),"%"), " ", ""))) 
          #scale_fill_discrete(labels=paste(data2()$taxon, " ", round(data2()$mean_rel_abund, 1),"%"))
      }
    ##############################
      piechart <- function(){
